...
def lzss_compress_literals(uncompressed: bytes) -> bytes:
    """Encode a valid Falcon/BMS LZSS stream using literals only."""
    if not uncompressed:
        return b""
    ...
    while cursor < len(uncompressed):
        chunk = uncompressed[cursor : cursor + 8]
...
def lzss_compress(uncompressed: bytes) -> bytes:
    ...
    while cursor < size:
        ...
        for bit_index in range(8):
            ...
            if best_length >= BREAK_EVEN + 1 and best_start >= 0:
...
def _remember_positions(
    ...
    for position in range(start, min(start + length, limit)):
        key = bytes(uncompressed[position : position + 2])
        recent_by_key[key].append(position)
